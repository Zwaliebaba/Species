...
void Clouds::RenderQuad(float posNorth, float posSouth, float posEast, float posWest, float height, float texNorth, float texSouth, float texEast,
                        float texWest)
{
  int const steps = 4;
...
void Clouds::RenderBlobby(float _predictionTime)
{
  ...
  if (cloudDetail == 1)
  {
    RenderQuad(zStart, zEnd, xStart, xEnd, height, offset.x, detail + offset.x, offset.z, detail + offset.z);
  }
