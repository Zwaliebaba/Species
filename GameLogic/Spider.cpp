...
namespace Species
{
  ...
  void Spider::StompFoot(DirectX::XMFLOAT3 const& _pos)
  {
    ...
    for (int i = 0; i < numFound; ++i)
    {
      ...
      float distance = DirectX::XMVectorGetX(
        DirectX::XMVector3Length(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&entity->m_pos), DirectX::XMLoadFloat3(&_pos))));
      ...
      DirectX::XMStoreFloat3(&push, DirectX::XMVectorScale(DirectX::XMVector3Normalize(DirectX::XMVectorSubtract(
                                                             DirectX::XMLoadFloat3(&entity->m_pos), DirectX::XMLoadFloat3(&_pos))),
                                                           fraction));
  ...
  bool Spider::FaceTarget()
  {
    ...
    if (dotProd < 0.999f)
    {
      ...
      DirectX::XMVECTOR const rotation = NearlyEquals(axisLength, 0.0f) ? DirectX::XMVectorSet(turnRate, 0.0f, 0.0f, 0.0f)
                                                                        : DirectX::XMVectorScale(rotationAxis, turnRate / axisLength);
      ...
      if (g_location->m_spirits.ValidIndex(i))
      {
        ...
        if (s->NumNearbyEggs() < 3 && s->m_pos.y > 10)
        {
          float theDist = DirectX::XMVectorGetX(
            DirectX::XMVector3Length(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&s->m_pos), DirectX::XMLoadFloat3(&m_pos))));
