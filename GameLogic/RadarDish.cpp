...
bool RadarDish::Advance()
{
  ...
  if (m_horizontallyAligned && !m_verticallyAligned)
  {
    ...
    // is simulation code, so it converts back rather than going native: row 0
    // of the transform is what Matrix34 called r. It converts properly in T17.
    m_dish->m_angVel = Vector3(m_dish->m_transform._11, m_dish->m_transform._12, m_dish->m_transform._13) * amount;

    m_verticallyAligned = (Vector3(m_dish->m_angVel).Mag() < 0.001f);
  }

  Matrix34 upperMountTransform(m_upperMount->m_transform);
  upperMountTransform.RotateAround(Vector3(m_upperMount->m_angVel) * SERVER_ADVANCE_PERIOD);
  m_upperMount->m_transform = upperMountTransform;

  Matrix34 dishTransform(m_dish->m_transform);
  dishTransform.RotateAround(Vector3(m_dish->m_angVel) * SERVER_ADVANCE_PERIOD);
  m_dish->m_transform = dishTransform;

  if (m_movementSoundsPlaying && m_horizontallyAligned && Vector3(m_dish->m_angVel).Mag() < 0.05f)
  {
...
Vector3 RadarDish::GetEndPoint() { return GetDishPos(0.0f) + GetDishFront(0.0f) * m_range; }
