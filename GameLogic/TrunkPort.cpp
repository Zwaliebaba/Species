    ...
    if (m_openTimer > 0.0f)
    {
      ...
      for (int x = 0; x < m_heightMapSize; ++x)
      {
        for (int z = 0; z < m_heightMapSize; ++z)
        {
          ...
          thisDif = DirectX::XMVectorMultiplyAdd(
            DirectX::g_XMIdentityR1, DirectX::XMVectorReplicate(cosf(g_gameTime) * wave1 * timeScale * 10 * (1.0f - fractionOut)), thisDif);
      ...
      for (int x = 0; x < m_heightMapSize - 1; ++x)
      {
        for (int z = 0; z < m_heightMapSize - 1; ++z)
        {
          ...
          DirectX::XMVECTOR const thisPos3 = DirectX::XMVectorAdd(DirectX::XMLoadFloat3(&m_heightMap[(z + 1) * m_heightMapSize + x + 1]),
                                                                  DirectX::XMLoadFloat3(&difMap[x + 1][z + 1]));
