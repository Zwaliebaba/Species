...
namespace Species
{
  ...
  // *** GetGridIndexX
  int EntityGrid::GetGridIndexX(float _worldX) { return (int)(_worldX * m_cellSizeXRecip); }


  // *** GetGridIndexZ
  int EntityGrid::GetGridIndexZ(float _worldZ) { return (int)(_worldZ * m_cellSizeZRecip); }
  ...
  // *** UpdateObject
  void EntityGrid::UpdateObject(WorldObjectId _objectId, float _oldWorldX, float _oldWorldZ, float _newWorldX, float _newWorldZ, float _radius)
  {
    ...
    for (z = 0; z < m_numCellsZ; ++z)
    {
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        ...
        if (numEntities > 0)
        {
          ...
          g_editorFont.DrawText3DCentre(DirectX::XMFLOAT3(worldX, worldY, worldZ), 5.0f, "{}", numEntities);
        }
      }
    }
  }
