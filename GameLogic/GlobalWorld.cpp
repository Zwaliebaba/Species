...
namespace Species
{
  ...
  {
    const char* names[] = {"Citizen",    "Officer",   "Squad",  "Laser",       "Grenade", "Rocket",
                           "Controller", "AirStrike", "Armour", "TaskManager", "Engineer"};
  ...
  void SphereWorld::RenderHeaven()
  {
    ...
    for (int i = 0; i < 50; ++i)
    {
      DirectX::XMVECTOR const pos =
        DirectX::XMVectorSet(sinf(i / g_gameTime + i) * 20, sinf(g_gameTime + i) * i, cosf(i / g_gameTime + i) * 20, 0.0f);
