...
namespace Species
{
  ...
  void Virii::RecordHistoryPosition(bool _required)
  {
    ...
    if (static_cast<int>(m_positionHistory.size()) > 0)
    {
      ...
      DirectX::XMStoreFloat3(&history->m_glowDiff, NearlyEquals(history->m_distance, 0.0f)
                                                     ? DirectX::XMVectorSet(10.0f, 0.0f, 0.0f, 0.0f)
  ...
  bool Virii::SearchForEnemies()
  {
    ...
    WorldObjectId bestEnemyId =
      g_location->m_entityGrid->GetBestEnemy(m_pos.x, m_pos.z, VIRII_MINSEARCHRANGE, VIRII_MAXSEARCHRANGE, m_id.GetTeamId());
  ...
  bool Virii::SearchForIdleDirection()
  {
    ...
    if (chanceOfReturn >= 1.0f || syncfrand(1.0f) <= chanceOfReturn)
    {
      ...
      DirectX::XMStoreFloat3(&nextPos,
                             DirectX::XMVectorMultiplyAdd(step, DirectX::XMVectorReplicate(m_retargetTimer), DirectX::XMLoadFloat3(&m_pos)));
