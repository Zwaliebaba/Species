...
namespace Species
{
  ...
  */
  ...
  void InsertionSquad::SetWayPoint(DirectX::XMFLOAT3 const& _pos)
  {
    ...
    if (controller)
    {
      DirectX::XMFLOAT3 const lastAddedPos =
        controller->m_route->m_wayPoints[static_cast<int>(controller->m_route->m_wayPoints.size()) - 1]->GetPos();
    ...
    while (i < static_cast<int>(m_positionHistory.size()) && _distFromPointMan > 0.0f)
    {
      ...
      if (magDelta > _distFromPointMan)
      {
        ...
        DirectX::XMStoreFloat3(&result,
                               DirectX::XMVectorMultiplyAdd(DirectX::XMVector3Normalize(delta), DirectX::XMVectorReplicate(_distFromPointMan),
  ...
  void Squadie::RunAI()
  {
    ...
    {
      float distance = DirectX::XMVectorGetX(
        DirectX::XMVector3Length(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&enemy->m_pos), DirectX::XMLoadFloat3(&m_pos))));
