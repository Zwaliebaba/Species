...
namespace Species
{
  ...
  bool ThrowableWeapon::Advance()
  {
    if (m_type != EffectThrowableAirstrikeMarker && syncfrand() > 0.2f)
    {
      ...
      DirectX::XMStoreFloat3(&trailPos, DirectX::XMVectorNegativeMultiplySubtract(DirectX::XMLoadFloat3(&m_vel),
                                                                                  DirectX::XMVectorReplicate(SERVER_ADVANCE_PERIOD),
  ...
  void Rocket::Render(float predictionTime)
  {
    ...
    for (int i = 0; i < 5; ++i)
    {
      ...
      DirectX::XMStoreFloat3(&pos, DirectX::XMVectorNegativeMultiplySubtract(DirectX::XMLoadFloat3(&m_vel),
                                                                             DirectX::XMVectorReplicate(0.05f + frand(0.05f)), predictedPos));
      pos.x += sfrand(8.0f);
      pos.y += sfrand(8.0f);
      pos.z += sfrand(8.0f);
  ...
  bool Laser::Advance()
  {
    ...
    if (m_pos.y <= landHeight)
    {
      ...
      else
      {
        ...
        float distanceTravelled = DirectX::XMVectorGetX(
          DirectX::XMVector3Length(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&hitPoint), DirectX::XMLoadFloat3(&oldPos))));
  ...
  void Shockwave::Render(float predictionTime)
  {
    ...
    if (m_size - predictedLife < 1.0f)
    {
      ...
      float distToBang = DirectX::XMVectorGetX(
        DirectX::XMVector3Length(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&camPosStore), DirectX::XMLoadFloat3(&m_pos))));
  ...
  bool TurretShell::Advance()
  {
    ...
    float radius =
      DirectX::XMVectorGetX(DirectX::XMVector3Length(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&m_pos), DirectX::XMLoadFloat3(&oldPos)))) /
      1.0f;
    int numFound;
