...
void Flag::Initialise()
{
  for (int x = 0; x < FLAG_RESOLUTION; ++x)
  {
    ...
    {
      Vector3 targetPos = m_pos + (float)x / (float)FLAG_RESOLUTION * m_front + (float)z / (float)FLAG_RESOLUTION * m_up;
...
void Flag::SetPosition(Vector3 const& _pos) { m_pos = _pos; }
...
void Flag::Render()
{
  ...
  for (int x = 0; x < FLAG_RESOLUTION; ++x)
  {
    for (int z = 0; z < FLAG_RESOLUTION; ++z)
    {
      ...
      if (x == 0)
      {
        targetPos = m_pos + m_up * m_size / 2.0f + (float)x / (float)(FLAG_RESOLUTION - 1) * m_front * m_size +
                    (float)z / (float)(FLAG_RESOLUTION - 1) * m_up * m_size;
      }
      else
      {
        targetPos = m_flag[x - 1][z] + 1.0f / (float)(FLAG_RESOLUTION - 1) * m_front * m_size;
      }
