...
namespace Species
{
  ...
  bool Citizen::SearchForThreats()
  {
    ...
    if (entity && !entity->m_dead)
    {
      ...
      if (entity->m_type == TypeSporeGenerator || entity->m_type == TypeTripod || entity->m_type == TypeSpider ||
          entity->m_type == TypeSoulDestroyer || entity->m_type == TypeTriffidEgg || entity->m_type == TypeInsertionSquadie ||
  ...
  DirectX::XMFLOAT3 Citizen::PushFromObstructions(DirectX::XMFLOAT3 const& pos, bool killem)
  {
    ...
    DirectX::XMStoreFloat4x4(&transform,
                             BasisFromFrontAndUp(DirectX::XMLoadFloat3(&m_front), DirectX::g_XMIdentityR1, DirectX::XMLoadFloat3(&result)));
  ...
  void Citizen::TakeControl(int _controllerId)
  {
    ...
    if (controller)
    {
      ...
      DirectX::XMStoreFloat3(&m_wayPoint,
                             DirectX::XMVectorAdd(DirectX::XMLoadFloat3(&m_wayPoint), DirectX::XMVectorSet(jitterX, 0.0f, jitterZ, 0.0f)));
      m_wayPoint = PushFromObstructions(m_wayPoint);
      m_wayPoint.y = g_location->m_landscape.m_heightMap->GetValue(m_wayPoint.x, m_wayPoint.z);
