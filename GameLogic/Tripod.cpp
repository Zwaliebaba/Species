...
namespace Species
{
  ...
  void Tripod::DoFallForTwoLegs()
  {
    ...
    {
      DirectX::XMStoreFloat3(&footToFootStore, DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&m_legs[1]->m_foot.m_pos),
                                                                         DirectX::XMLoadFloat3(&m_legs[2]->m_foot.m_pos)));
    ...
    {
      DirectX::XMStoreFloat3(&footToFootStore, DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&m_legs[2]->m_foot.m_pos),
                                                                         DirectX::XMLoadFloat3(&m_legs[0]->m_foot.m_pos)));
      RayRayDist(m_legs[0]->m_foot.m_pos, footToFootStore, m_pos, worldUp, &betweenFeetStore, &underBodyStore);
    }
    else
    {
      DirectX::XMStoreFloat3(&footToFootStore, DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&m_legs[0]->m_foot.m_pos),
                                                                         DirectX::XMLoadFloat3(&m_legs[1]->m_foot.m_pos)));
      RayRayDist(m_legs[0]->m_foot.m_pos, footToFootStore, m_pos, worldUp, &betweenFeetStore, &underBodyStore);
    }
  ...
  void Tripod::AdvanceWalk()
  {
    ...
    // Fall
    {
      bool allOnGround = m_legs[0]->m_foot.m_state == EntityFoot::FootState::OnGround &&
                         m_legs[1]->m_foot.m_state == EntityFoot::FootState::OnGround && m_legs[2]->m_foot.m_state == EntityFoot::FootState::OnGround;
      if (!allOnGround)
      {
        DoFallForTwoLegs();
      }
  ...
  bool Tripod::Advance(Unit* _unit)
  {
    ...
    DirectX::XMStoreFloat3(
      &m_front, DirectX::XMVector3TransformNormal(DirectX::XMLoadFloat3(&m_front), DirectX::XMMatrixRotationY(g_advanceTime * m_angVel.y)));
