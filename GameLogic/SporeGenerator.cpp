...
void SporeGenerator::Render(float _predictionTime)
{
  ...
  for (int i = 0; i < SPOREGENERATOR_NUMTAILS; ++i)
  {
    ...
    for (int j = 0; j < numTailParts; ++j)
    {
      ...
      thisTailPos += Vector3(sinf(timeIndex) * 10.0f, sinf(timeIndex) * 10.0f, sinf(timeIndex) * 10.0f);
      ...
      if (m_state == StatePanic)
      {
        ...
        thisTailPos += Vector3(cosf(g_gameTime * panicFraction + i + j) * 5.0f, cosf(g_gameTime * panicFraction + i + j) * 5.0f,
