...
namespace Species
{
  ...
  static bool SquaddieSelected()
  {
    ...
    return TheCamera()->IsInMode(Camera::Mode::ModeEntityTrack) && (unit = GetSelectedUnit()) && unit->m_troopType == Entity::TypeInsertionSquadie &&
  ...
  bool ControlHelpSystem::CheckCondition(int _condition)
  {
    ...
    case CondMoveCameraOrUnit:
      return !TheTaskManagerInterface()->m_visible &&
             (TheCamera()->IsInMode(Camera::Mode::ModeFreeMovement) || TheCamera()->IsInMode(Camera::Mode::ModeEntityTrack));

    case CondCameraAim:
      return !TheTaskManagerInterface()->m_visible && TheCamera()->IsInMode(Camera::Mode::ModeFreeMovement);
    ...
    case CondFireGrenades:
      return !TheTaskManagerInterface()->m_visible && g_inputManager->controlEvent(ControlType::ControlUnitPrimaryFireDirected) &&
             SquaddieSelected() && WeaponSelected(GlobalResearch::TypeGrenade);

    case CondFireRocket:
      return !TheTaskManagerInterface()->m_visible && g_inputManager->controlEvent(ControlType::ControlUnitPrimaryFireDirected) &&
             SquaddieSelected() && WeaponSelected(GlobalResearch::TypeRocket);

    case CondFireAirstrike:
      return !TheTaskManagerInterface()->m_visible && g_inputManager->controlEvent(ControlType::ControlUnitPrimaryFireDirected) &&
             SquaddieSelected() && WeaponSelected(GlobalResearch::TypeAirStrike);
