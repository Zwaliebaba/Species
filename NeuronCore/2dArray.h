...
template <class T> inline T const& Array2D<T>::GetData(unsigned short _x, unsigned short _y) const
{
...
template <class T> inline T* Array2D<T>::GetPointer(unsigned short _x, unsigned short _y)
{
  if (_x >= m_numColumns || _y >= m_numRows)
    return &m_outsideValue;
  return &(m_data[_y * m_numColumns + _x]);
...
template <class T> inline T const* Array2D<T>::GetConstPointer(unsigned short _x, unsigned short _y) const
{
  if (_x >= m_numColumns || _y >= m_numRows)
    return &m_outsideValue;
  return &(m_data[_y * m_numColumns + _x]);
...
template <class T> inline void Array2D<T>::PutData(unsigned short _x, unsigned short _y, T const& _value)
{
  if (_x >= m_numColumns || _y >= m_numRows)
    return;
...
template <class T> inline void Array2D<T>::AddToData(unsigned short _x, unsigned short _y, T const& _value)
{
  if (_x >= m_numColumns || _y >= m_numRows)
    return;
  m_data[_y * m_numColumns + _x] += _value;
...
template <class T> inline unsigned short Array2D<T>::GetNumRows() const { return m_numRows; }


template <class T> inline unsigned short Array2D<T>::GetNumColumns() const { return m_numColumns; }


template <class T> void Array2D<T>::SetAll(T const& _value)
{
