...
class SoftwareChannel
{
  public:
    ...
    bool m_forceVolumeJump; // True when a channel has just had a new sound effect assigned to it - this prevents the volume ramping code from being
                            // used when a new sound first starts
...
void SoundLibrary3dSoftware::Advance() { g_soundLibrary2d->TopupBuffer(); }
  ...
  START_PROFILE(m_profiler, "GetChannelData");
  {
    ...
    for (int i = 0; i < m_numChannels; ++i)
    {
      ...
      if (i == m_musicChannelId)
      {
        if (m_musicCallback)
        {
          m_channels[i].m_containsSilence = !m_musicCallback(m_channels[i].m_buffer, samplesNeeded, &silenceRemaining);
        }
        else
        {
      ...
      else
      {
        if (m_mainCallback)
        {
          m_channels[i].m_containsSilence = !m_mainCallback(i, m_channels[i].m_buffer, samplesNeeded, &silenceRemaining);
        }
        else
        {
          m_channels[i].m_containsSilence = true;
...
void SoundLibrary3dSoftware::ApplyDspFX(float _duration)
{
  START_PROFILE(m_profiler, "ApplyDspFX");
  {
    for (int i = 0; i < m_numChannels; ++i)
    {
      ...
      {
        if (m_channels[i].m_dspFX[j].m_userFilter == nullptr)
          continue;
        ...
        m_channels[i].m_dspFX[j].m_userFilter->Process(m_channels[i].m_buffer, samplesNeeded);
      }
    }
  }
...
void SoundLibrary3dSoftware::CalcChannelVolumes(int _channelIndex, float* _left, float* _right)
{
  ...
  if (calculatedVolume < -10.0f)
    calculatedVolume = -10.0f;
...
void SoundLibrary3dSoftware::MixSameFreqFixedVol(signed short* _inBuf, unsigned int _numSamples, float _volLeft, float _volRight)
{
...
void SoundLibrary3dSoftware::MixDiffFreqFixedVol(signed short* _inBuf, unsigned int _numSamples, float _volLeft, float _volRight, float _relativeFreq)
{
  float* left = m_left;
  float* right = m_right;
  int nearestSample;
...
void SoundLibrary3dSoftware::MixSameFreqRampVol(signed short* _inBuf, unsigned int _numSamples, float _volL1, float _volR1, float _volL2,
                                                float _volR2)
{
...
void SoundLibrary3dSoftware::MixDiffFreqRampVol(signed short* _inBuf, unsigned int _numSamples, float _volL1, float _volR1, float _volL2,
                                                float _volR2, float _relativeFreq)
{
  float volLeft = _volL1;
  float volRight = _volR1;
  float volLeftInc = (_volL2 - _volL1) / (float)_numSamples;
...
void SoundLibrary3dSoftware::Callback(StereoSample* _buf, unsigned int _numSamples)
{
  ...
  for (int i = 0; i < m_numChannels; ++i)
  {
    ...
    if (!m_channels[i].m_containsSilence)
    {
      ...
      else
      {
        if (NearlyEquals(relativeFreq, 1.0f))
        {
          MixSameFreqRampVol(inBuf, _numSamples, m_channels[i].m_oldVolLeft, m_channels[i].m_oldVolRight, volLeft, volRight);
        }
        else
        {
          MixDiffFreqRampVol(inBuf, _numSamples, m_channels[i].m_oldVolLeft, m_channels[i].m_oldVolRight, volLeft, volRight, relativeFreq);
        }
      }
    }
  ...
  {
    if (fabsf(m_left[i]) > largest)
      largest = fabsf(m_left[i]);
    if (fabsf(m_right[i]) > largest)
      largest = fabsf(m_right[i]);
  }
...
int SoundLibrary3dSoftware::GetChannelBufSize(int _channel) const { return 0; }
...
void SoundLibrary3dSoftware::SetChannelMinDistance(int _channel, float _minDistance) { m_channels[_channel].m_minDist = _minDistance; }
...
void SoundLibrary3dSoftware::ResetChannel(int _channel) { m_channels[_channel].m_forceVolumeJump = true; }
