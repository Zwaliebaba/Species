...
class InputFilterManager
{
  ...
  public:
    ...
    bool parseFilterSpecString(std::string const& description, InputFilterSpec& spec, std::string& err);

    // Turn a set of tokens into an InputFilterSpec. Not for general use outside of the input system
    bool parseFilterSpecTokens(InputSpecTokens const& tokens, InputFilterSpec& spec, std::string& err);

    // Apply a filter to a set of InputSpec to get an InputDetails
    bool filter(InputSpecList const& inDetails, InputFilterSpec const& filterSpec, InputDetails& outDetails);

    // Apply a filter to a set of InputDetails to get another InputDetails
    bool filter(InputDetailsList const& inDetails, InputFilterSpec const& filterSpec, InputDetails& outDetails);
