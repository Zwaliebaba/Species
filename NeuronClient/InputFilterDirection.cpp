...
#include "InputFilterDirection.h"
...
namespace Neuron
{
  ...
  bool DirectionInputFilter::filter(InputDetailsList const& inDetails, InputFilterSpec const& filterSpec, InputDetails& outDetails) { return false; }


  // Return true if the tokens were successfully parsed into an InputFilterSpec
  bool DirectionInputFilter::parseFilterSpecification(InputSpecTokens const& tokens, InputFilterSpec& spec)
  {
    if (tokens.length() != 3)
      return false;
